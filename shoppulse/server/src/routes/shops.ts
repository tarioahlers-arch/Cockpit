...
import { db, getShop, invalidateEvents, recentEvents, type ExperimentRow, type ShopRow } from '../db/index.js';
...
shopsRouter.post('/', (req, res) => {
  ...
  const info = db
    .prepare('INSERT INTO shops (name, domain, platform, niche, public_key) VALUES (?, ?, ?, ?, ?)')
    .run(name.trim(), cleanDomain, p, typeof niche === 'string' && niche.trim() ? niche.trim() : 'allgemein', key);
  res.status(201).json(getShop(Number(info.lastInsertRowid)));
});

shopsRouter.post('/demo', (_req, res) => {
  res.status(201).json(createDemoShop());
});
