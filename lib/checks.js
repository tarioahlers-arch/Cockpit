...
//
// Die Nudge-Bezeichnungen folgen den im Strategiebericht "Erschließung des
// Online-Marktes" (Kap. 7.1/7.2) hergeleiteten Prinzipien nach Kahneman
// (Verlustaversion, System 1/System 2, Framing, Ankereffekt) und Thaler
// (Default-Effekt, Mental Accounting, Social Proof, Verknappung).
// Kapitel 7.3 verlangt zusätzlich die Gegenrichtung: Die Kategorie
// "fairness" prüft auf manipulative Muster (Sludge/Dark Patterns), statt
// Verknappung und Dringlichkeit ungeprüft zu belohnen.

export const CATEGORIES = {
  friction: "Kauf-Friktion & Checkout",
  trust: "Vertrauen & Sicherheit",
  fairness: "Fairness & Transparenz",
...
// Fairness-Kriterien sind invertiert: Sie bestehen, wenn KEIN Hinweis auf ein
// manipulatives Muster gefunden wurde. Ein Fund ist ein Prüfauftrag für den
// menschlichen Test, keine abschließende Feststellung.
export const INVERTED_CATEGORIES = new Set(["fairness"]);
...
export const CHECKS = [
  ...
  },
  {
    ...
    nudge: "Social Proof & Verknappung (Thaler)",
    impact: { min: 0.01, max: 0.02 },
    passRatio: (ctx) =>
      matchAny(ctx, [
  ...
  // ---------- Preisdarstellung & Anchoring ----------
  {
    ...
    nudge: "Ankereffekt (Kahneman/Tversky)",
    impact: { min: 0.01, max: 0.03 },
    passRatio: (ctx) =>
      anyDoc(
        ctx,
  ...
  },
  {
    ...
    nudge: "System 1/System 2 (Kahneman)",
    impact: { min: 0.005, max: 0.015 },
    passRatio: (ctx) =>
  ...
  // ---------- Kauf-Friktion & Checkout ----------
  {
    ...
    nudge: "Default-Effekt & Status-quo-Bias (Thaler)",
    impact: { min: 0.02, max: 0.05 },
    passRatio: (ctx) =>
  ...
  },
  {
    ...
    nudge: "Verlustaversion (Kahneman)",
    impact: { min: 0.01, max: 0.03 },
    passRatio: (ctx) =>
      matchAny(ctx, [
...

  {
    ...
    category: "friction",
    weight: 3,
    label: "Versandkosten früh und eindeutig ausgewiesen",
    nudge: "Mental Accounting (Thaler)",
    impact: { min: 0.02, max: 0.06 },
    passRatio: (ctx) =>
      matchAny(ctx, [
        "versandkostenfrei",
        "kostenloser versand",
        /zzgl\.?\s*versandkosten/,
        /inkl\.?\s*versandkosten/,
        /versandkosten:?\s*(\d|ab|kostenlos)/,
      ])
        ? 1
        : 0,
    tip: () =>
      "Versandkosten bereits auf Produkt- und Warenkorbseite beziffern, nicht erst im letzten Checkout-Schritt. Unerwartete Zusatzkosten sind mit 39% der meistgenannte Abbruchgrund überhaupt.",
  },
  {
    id: "progress_indicator",
    category: "friction",
    weight: 2,
    label: "Fortschrittsanzeige im Bestellprozess",
    nudge: "System 1/System 2 (Kahneman)",
    impact: { min: 0.01, max: 0.03 },
    passRatio: (ctx) =>
      anyDoc(ctx, '[class*="progress" i], [class*="step-indicator" i], [class*="checkout-step" i], ol[class*="step" i]') ||
      matchAny(ctx, [/schritt\s*\d\s*(von|\/)\s*\d/, /step\s*\d\s*(of|\/)\s*\d/])
        ? 1
        : 0,
    tip: () =>
      "Eine sichtbare Schrittanzeige („Schritt 2 von 3“) im Checkout ergänzen. Sie macht den verbleibenden Aufwand kalkulierbar und verhindert den Abbruch aus Ungewissheit.",
  },
  {
    id: "lean_forms",
    category: "friction",
    weight: 2,
    label: "Schlanke Formulare (wenige Pflichtfelder)",
    nudge: "System 1/System 2 (Kahneman)",
    impact: { min: 0.01, max: 0.04 },
    passRatio: (ctx) => {
      const required = ctx.docs.reduce(
        ($sum, $) => $sum + $("input[required], select[required], textarea[required]").length,
        0
      );
      // Ohne Pflichtfelder auf den geprüften Seiten gibt es nichts zu
      // bemängeln - bewertbar wird das erst mit einer Checkout-URL.
      if (required === 0) return 1;
      if (required <= 8) return 1;
      if (required <= 12) return 0.6;
      if (required <= 16) return 0.3;
      return 0;
    },
    tip: (ctx) => {
      const required = ctx.docs.reduce(
      ...
      return `Auf den geprüften Seiten wurden ${required} Pflichtfelder gefunden. Abfragen auf das für Lieferung und Zahlung Notwendige reduzieren; jedes zusätzliche Feld erhöht die Abbruchwahrscheinlichkeit.`;
    },
  },

  // ---------- Fairness & Transparenz (invertiert) ----------
  {
    id: "no_precheck_boxes",
    category: "fairness",
    weight: 3,
    label: "Keine vorausgewählten Zusatzoptionen",
    nudge: "Faire Choice Architecture (Thaler)",
    impact: { min: 0, max: 0 },
    passRatio: (ctx) =>
      anyDoc(ctx, 'input[type="checkbox"][checked], input[type="checkbox"][checked="checked"]') ? 0 : 1,
    tip: () =>
      "Es wurden vorausgewählte Checkboxen gefunden. Prüfen, ob dabei Zusatzleistungen, Newsletter oder Versicherungen vorangekreuzt sind: Das gilt als Dark Pattern, ist bei kostenpflichtigen Zusatzleistungen rechtlich unzulässig und beschädigt das Vertrauen.",
  },
  {
    id: "no_hidden_fees",
    category: "fairness",
    weight: 3,
    label: "Keine spät auftauchenden Zusatzgebühren",
    nudge: "Mental Accounting (Thaler)",
    impact: { min: 0, max: 0 },
    passRatio: (ctx) =>
      matchAny(ctx, [
        ...
        : 1,
    tip: () =>
      "Es wurden Begriffe für Zusatzgebühren gefunden. Prüfen, an welcher Stelle im Bestellprozess diese erstmals sichtbar werden. Tauchen sie erst im letzten Schritt auf, wirken sie als Verlust und treiben die Abbruchrate.",
  },
  {
    id: "no_subscription_trap",
    category: "fairness",
    weight: 2,
    label: "Keine unklare automatische Vertragsverlängerung",
    nudge: "Sludge-Vermeidung (Thaler)",
    impact: { min: 0, max: 0 },
    passRatio: (ctx) =>
      matchAny(ctx, [
        ...
        "geht automatisch über in",
      ])
        ? 0
        : 1,
    tip: () =>
      "Es wurden Hinweise auf automatische Verlängerungen gefunden. Prüfen, ob Laufzeit, Folgekosten und Kündigungsweg vor Vertragsschluss gleich deutlich sichtbar sind wie das Einstiegsangebot.",
  },
  {
    id: "no_confirmshaming",
    category: "fairness",
    weight: 2,
    label: "Kein Confirmshaming in Dialogen",
    nudge: "Faire Choice Architecture (Thaler)",
    impact: { min: 0, max: 0 },
    passRatio: (ctx) =>
      matchAny(ctx, [
        /nein danke,? ich (will|möchte|brauche)/,
        /ich (will|möchte) nicht sparen/,
        /kein interesse an (rabatt|vorteil)/,
        /ich verzichte (gerne )?auf/,
      ])
        ? 0
        : 1,
    tip: () =>
      "Es wurden abwertend formulierte Ablehn-Optionen gefunden („Nein danke, ich möchte nicht sparen“). Solche Formulierungen beschämen Nutzer für eine legitime Wahl. Neutral formulieren („Nein danke“ / „Schließen“).",
  },
